function H=Hstark2D_B_os(E,r,fl,Pl,Flist,B)
...
%% Constants
ub=9.27400899e-28;    %Bohr magneton in J/G
...
[Fvec,Ften]=Fmat(Flist);    %Gets the F components for hamiltonian. Be carefull, it has some normalization factors

%% Magnetic field energy for each hyperfine state

for k=1:numel(Flist);
    Hinit{k}=2*Flist(k)*B.gf(k)*ub*BF(B.B,Fvec(k,:));   %factor added to renormalize F operator
end
...
for ix=1:numel(r.x);
    for iy=1:numel(r.y);
            ...
            for w=1:numel(fl);  %loop over wavelengths
                ...
                Ep=sqrt(Pl(w))*(1/2)*[E.x(ix,iy,w),E.y(ix,iy,w),E.z(ix,iy,w)];
                Em=sqrt(Pl(w))*(1/2)*[conj(E.x(ix,iy,w)),conj(E.y(ix,iy,w)),conj(E.z(ix,iy,w))];
                ...
                for k=1:numel(Flist);   %start loop over possible hyperfine levels
                    if isempty(Hmat{k})==1  %Start matrix with magnetic field energy
                        Hmat{k}=Hinit{k}; %B.gf(k)*ub*BF(B.B,Fvec(k,:));
                    end
                    
                    %comupte the hamiltonian matrix for each k in the |F,mF>
                    %basis defined by the total angular momentum operator
                    Hmat{k}=Hmat{k}-alpha_scalar(w,k)*E2scalar*eye(2*Flist(k)+1);%-1i*alpha_vector(w,k)*FEv{k}-alpha_tensor(w,k)*FEt{k};
                    if w==numel(fl);
                        H{ix,iy,k}=sort(real(eig(Hmat{k})));   %final diagonalization
                    end
...
%Magnetic field interaction, weak limit assumption
function BF=BF(B,F);
BF=B(1)*F{1}+B(2)*F{2}+B(3)*F{3};
